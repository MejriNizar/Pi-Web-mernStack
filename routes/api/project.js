...
router.get("/activatedProject", async (req, res) => {
  try {
    const projects = await Project.find({ activated: true })
      .populate("documentation", ["label"])
      .populate("projectOwner", ["name"])
      .populate("group", ["name"])
      .sort({ creationDate: -1 });
...
router.post(
  ...
  [
    ...
    [
      ...
      check("description", "enter a description with 20 or greater").isLength({
        min: 20,
      }),
  ...
  async (req, res) => {
    ...
    if (numberGroups) projectFileds.settings.numberGroups = numberGroups;
    if (numberTolerence)
      projectFileds.settings.numberTolerence = numberTolerence;

    if (skills) projectFileds.settings.skills = skills;

    if (dueDate) projectFileds.settings.dueDate = dueDate;

    if (votingSystem) projectFileds.settings.votingSystem = votingSystem;
